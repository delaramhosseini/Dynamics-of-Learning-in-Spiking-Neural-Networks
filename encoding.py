...
class TimeToFirstSpikes(Behavior):
...
class PoissonDistribution(Behavior):
    def initialize(self, ng):
        ...
        min_data = self.data.min()
        self.range = self.parameter("range", None) or (min_data, max_data)
        ng.network.number_of_data = self.number_of_data
    ...
    def encode_poisson(self, ng, d):
        ...
        for i in range(num_neurons):
            spikes_times = np.random.poisson(data[i], self.input_time)
...
class NumericalCoding(Behavior):
    def initialize(self, ng):
        ...
        ng.network.inp_duration = int(
            self.parameter("inp_duration", default=1000000) / ng.network.dt
        )
    ...
    def forward(self, ng):
        ng.spikes = torch.logical_or(
            (
                ng.encoded_matrix[(ng.network.iteration - 1) % ng.network.inp_duration]
                if ng.network.iteration % ng.network.inp_duration <= self.time
                else ng.vector(0) != 0
            ),
            ng.spikes,
        )
...
class ResetMemory(Behavior):  # reset network memory
    def initialize(self, ng):
        self.neuron_groups = ng.network["NeuronGroup"]

    def forward(self, ng):
        if ng.network.iteration % ng.network.inp_duration == 1:
